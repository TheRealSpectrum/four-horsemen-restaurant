<template>
    <div class="container border border-current ">
        ...
        <div class="results flex flex-row flex-shrink-0 w-10/12 flex-wrap">
            <div class="w-1/3" v-for="item in computed_reservation_data" :key="item.index">
                <div v-if="isValid(item)" class="border-black border grid grid-cols-2 grid-rows-3 h-16 p-2 text-sm reservation relative">
                    <p class="row-start-1 col-start-1 leading-none" :title="item.name">{{(item.name.substring(0,15) + ((item.name.length>15)?"...":""))}}</p>
        ...
        <div class="edit">
            <p>reservation id : {{selectedID}}<input type="text" name="id" id="selectedID" placeholder="reservation id"  :value="selectedID" hidden></p>
            <label>guest phone<input type="text" name="id" id="phone" placeholder="reservation id" :value="selectedPhone"></label>
            <label>guest name<input type="text" name="id" id="name" placeholder="reservation id" :value="selectedName"></label>
            <label>guest count<input type="number" name="id" id="guestCount" placeholder="reservation id" :value="selectedGuestCount"></label>
            <label>event type<input type="text" name="id" id="event" placeholder="reservation id" :value="selectedEvent"></label>
            <label>Date<input type="date" name="Date" id="date" placeholder="reservation id" :value="selectedDate"></label>
            <label>notes
                <textarea type="text" name="Notes" id="notes" placeholder="reservation id" :value="selectedNotes">
                </textarea>
            </label>
            <label>Time<input type="time" name="Time" id="time" placeholder="reservation id" :value="selectedTime"></label>
            <label>tabels
                <div class="selectedTables">

                </div>
                <select type="time" name="Time" id="time" placeholder="reservation id" v-model="selectedTime" value="">
                    <option value="" v-for="table in table_data" :key="table.id">
